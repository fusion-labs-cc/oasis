...
def _relaunch_self(base: str, work: str) -> None:
    ...
    elif sys.platform == "darwin" and _relaunch_in_terminal(base, work, env):
        # A Terminal window now owns the fresh backend; the `open` launcher we
        # used has already exited, so there is no child of ours to health-check.
        return
    else:
...
def _relaunch_in_terminal(base: str, work: str, env: dict) -> bool:
    """Relaunch the backend in a visible Terminal window (macOS). Returns success.

    The first launch opens a Terminal through Finder's association with
    oasis-backend.command, so the user watches the "伺服器已就緒" banner and any
    errors scroll past. A plain Popen after an update instead runs the new backend
    headless in the background — the exact report this fixes: the update succeeds
    but no window ever appears. We reuse that same .command (banner, Chrome check,
    keep-open) so an updated relaunch looks identical to a double-click.

    `open -a Terminal` rather than osascript on purpose: osascript would drive
    Terminal.app over AppleEvents, which trips a one-time "允許控制 Terminal" TCC
    prompt mid-update. `open` merely launches and never prompts — but it also
    cannot pass environment through, so the wait-pid the light path needs
    (OASIS_UPDATE_WAIT_PID, so the new process holds off until this one exits) is
    baked into a tiny wrapper .command we write ourselves. A file we create carries
    no com.apple.quarantine attribute, so Gatekeeper never blocks it.

    Returns False — caller then falls back to a headless spawn — if the launcher
    the release ships next to the executable isn't there.
    """
    launcher = os.path.join(base, "oasis-backend.command")
    if not os.path.isfile(launcher):
        _log(f"no {launcher} for Terminal relaunch — falling back to headless")
        return False
    wait_pid = env.get(_WAIT_PID_ENV, "")
    wrapper = os.path.join(work, "relaunch.command")
    try:
        with open(wrapper, "w", encoding="utf-8") as f:
            f.write(
                "#!/bin/bash\n"
                "# Auto-generated by oasis updater.py — safe to delete.\n"
                f"export {_WAIT_PID_ENV}={shlex.quote(wait_pid)}\n"
        ...
        _log(f"Terminal relaunch failed ({exc}) — falling back to headless")
        return False
    _log(f"relaunched in Terminal via {wrapper} (open pid={proc.pid}, wait_pid={wait_pid or 'none'})")
    return True
...
cd "$BASE"
# Relaunch in a visible Terminal window, exactly like the initial double-click,
# so the user sees the "伺服器已就緒" banner instead of a headless background
# process (the whole point of shipping oasis-backend.command). `open` needs no
# Automation/TCC grant, unlike osascript. Fall back to a detached run if the
# launcher or `open` isn't available (e.g. a frozen build on a headless host).
LAUNCHER="$BASE/oasis-backend.command"
if [ -f "$LAUNCHER" ] && command -v open >/dev/null 2>&1; then
  chmod +x "$LAUNCHER" 2>/dev/null || true
  if open -a Terminal "$LAUNCHER"; then
    log "relaunched $EXENAME in Terminal"
  else
    log "open Terminal failed — relaunching $EXENAME headless"
    "$BASE/$EXENAME" >"$WORK/backend.log" 2>&1 &
    log "relaunched $EXENAME headless (pid $!)"
  fi
else
  log "no Terminal launcher — relaunching $EXENAME headless"
  "$BASE/$EXENAME" >"$WORK/backend.log" 2>&1 &
  log "relaunched $EXENAME headless (pid $!)"
fi
