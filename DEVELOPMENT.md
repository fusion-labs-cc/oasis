...
`sites.example.json` 裡有三個**選用**區塊，沒寫就完全不生效，行為與原本一致：

| 區塊 | 用途 |
|---|---|
| `listing` | 一個網址解析成**多筆**記錄（整季動畫的每一集）。`url_patterns` 決定某網址算不算列表頁——引擎無法從 DOM 判斷，因為列表頁與單集頁通常同網域、同模板。列舉只用一個瀏覽器工作階段走完（含 `next_page` 分頁），**不會逐一開啟單集頁**：標題列表頁上就有。`max_pages` 是硬性上限，不是偏好設定——同一種網址形狀往往也能列出「整季所有番」。設了 `series_name` 就會把整份列表收進一個同名**系列**，並依列表順序編成第 1..N 集。 |
| `code` | 從網址推導收藏庫的 `code`。對列表頁是必要的：`code` 欄位是 UNIQUE，動畫標題不會命中內建的番號樣式，而「標題前 20 字」的後備方案對長片名的每一集都相同，會讓各集互相覆蓋。 |
| `media` | **第二條下載路徑**，給提供單一漸進式 mp4（而非 HLS 播放清單）的網站：`media.mode: "http"`，未設定即為 `"hls"`。這類播放器頁面上根本沒有影片網址，只有一枚有時效的 token，要靠一支 API 換得網址**與 CDN 要求的 cookie**，所以 cookie 必須跟著下載一起帶。 |

如何取得某網站的選擇器與 m3u8／媒體擷取方式，是使用者自身的責任；請確保你對該網站的存取與內容使用符合其服務條款與所在地法律。
...
│   ├── crawler.py            # TS 分段下載核心邏輯（HLS 路徑）
│   ├── httpdl.py             # 單一檔案串流下載（漸進式 mp4 路徑，支援續傳）
│   ├── download.py           # Selenium 分析及下載流程（依 adapter 選 HLS／漸進式）
